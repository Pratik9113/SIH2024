...
export const faceRecognition = async (req, res) => {
    try {
        const { email_id } = req.body; // Use email_id from JWT payload
        if (!email_id) {
            console.error("Error: email_id not found in JWT payload");
            return res.json(
                responseFormatter(
                    ApiStatusCodes.BAD_REQUEST,
                    false,
                    null,
                    "Invalid token: email_id not found"
                )
            );
        }
        const { latitude, longitude } = req.body; // Get location from the request body
        ...
        console.log('Received request for user:', email_id);
        ...
        const readLoginStatus = () => {
            return new Promise((resolve, reject) => {
                fs.readFile(loginStatusPath, 'utf8', (err, data) => {
                    ...
                    resolve(data.trim()); // Trim whitespace from file content
                });
            });
        };
        ...
        if (loginStatus === `Logged in as ${email_id}`) {
            let userFaceRecord = await FaceRecognition.findOne({ email_id });
